package vault.transitions.slide
{
	...
	public class Transition_SlideRightFast extends TransitionType
	{
		...
		public function Transition_SlideRightFast( from:Bitmap, to:Bitmap )
		{
			super( from, to );
		}
		...
		override public function start():void
		{
			...
			tl.append( new TweenLite( _fromSprite, 0.5, { x : _fromSprite.width + ( ( _fromSprite.width >> 1 ) * scaleDownTo ) } ) );
		}
		
		private function startToAnim():void
		{
			var tl:TimelineLite = new TimelineLite( { onComplete : resetBitmaps } );
			tl.append( new TweenLite( _toSprite, 0.5, { x : _centreX } ) );
