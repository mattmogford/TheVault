package vault.transitions.slide
{
	...
	public class Transition_SlideBottomFast extends TransitionType
	{
		...
		public function Transition_SlideBottomFast( from:Bitmap, to:Bitmap )
		{
			super( from, to );
		}
		...
		override public function start():void
		{
			...
			_toSprite.x = _fromSprite.width >> 1;
			_toSprite.y = -( _toSprite.height >> 1 );
			
			var tl:TimelineLite = new TimelineLite();
			tl.append( new TweenLite( _fromSprite, 0.25, { scaleX : scaleDownTo, scaleY : scaleDownTo, onComplete : startToAnim } ) );
			tl.append( new TweenLite( _fromSprite, 0.5, { y : _fromSprite.height + ( ( _fromSprite.height >> 1 ) * scaleDownTo ) } ) );
		}
		
		private function startToAnim():void
		{
			var tl:TimelineLite = new TimelineLite( { onComplete : resetBitmaps } );
			tl.append( new TweenLite( _toSprite, 0.5, { y : _centreY } ) );
			tl.append( new TweenLite( _toSprite, 0.25, { scaleX : 1, scaleY : 1 } ) );
		}
